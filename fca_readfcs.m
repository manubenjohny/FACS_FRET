...
%         warndlg('Can''t find FCSdata start position... ');
    ...
    elseif ~isempty(findstr(char(fcsheader_main'),'@$')) 
        mnemonic_separator = '@';% CyAn Summit FCS3.0
    elseif ~isempty(findstr(char(fcsheader_main'),'/$')) 
        mnemonic_separator = '/';% Attune FCS3.0
    else 
    ...
    for i=1:fcshdr.NumOfPar
        ...
        % edit SRL
        if ~strcmpi(fcshdr.par(i).name,'Time')
            fcshdr.par(i).PMTV = get_mnemonic_value(['$P',num2str(i),'V'],fcsheader_main, mnemonic_separator);
        end
        % end edit
